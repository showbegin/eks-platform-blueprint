...
The blueprint's production target is multi-account (ADR-001). However, anyone evaluating it — an engineer reading the repo to decide whether it's worth adopting, or standing it up to try the patterns before committing — usually has access to a single AWS account at most.
